...
import { Search, RefreshCw, Loader2, Monitor } from 'lucide-react';

const ADMIN_EXTRA_PACKAGES = [
  { id: 'pacote_offline', label: 'Offline' },
  { id: 'pacote_concurso', label: 'Concurso' },
  { id: 'simulacao_oficial_extra', label: 'Sim Extra' },
];
...
export default function AdminUsers() {
  ...
  const fetchUsers = async () => {
    ...
    const { data, error } = await supabase
      .from('profiles')
      .select('*, areas(name), last_active, total_xp, active_packages')
  ...
  const handleUpdateUserArea = async (userId: string, areaId: string) => {
    ...
    if (error) alert('Erro ao atualizar area.');
    else void fetchUsers();
  };

  const handleToggleUserPackage = async (userId: string, currentPackages: string[], packageId: string) => {
    const packageSet = new Set((currentPackages || []).filter(Boolean));

    if (packageSet.has(packageId)) {
      packageSet.delete(packageId);
    } else {
    ...
    const { error } = await supabase
      .from('profiles')
      .update({ active_packages: [...packageSet] })
      .eq('id', userId);

    if (error) {
      alert('Erro ao atualizar pacote do utilizador.');
      return;
    }
          ...
          <table className="w-full text-left border-separate border-spacing-y-2">
            <thead className="bg-slate-50/50 text-slate-400 text-[9px] font-black uppercase tracking-[0.25em]">
              <tr>
                ...
                <th className="px-6 py-4 text-right rounded-r-2xl">Acoes</th>
              </tr>
            </thead>
              ...
              {loadingUsers ? (
                <tr>
                  <td colSpan={7} className="px-6 py-32 text-center">
                    <Loader2 className="h-10 w-10 animate-spin mx-auto text-emerald-500 mb-2" />
                    <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">A carregar...</p>
                  </td>
                </tr>
              ) : filteredUsers.length === 0 ? (
                <tr>
                  <td colSpan={7} className="px-6 py-32 text-center text-slate-400 font-bold uppercase tracking-widest italic opacity-40">
                    Nenhum rastro encontrado...
                  </td>
                </tr>
              ) : (
                filteredUsers.map((user) => {
                  const currentPackages = Array.isArray(user.active_packages) ? user.active_packages : [];

                  return (
                    <tr key={user.id} className="bg-slate-50/20 hover:bg-white transition-all group hover:shadow-xl hover:shadow-slate-200/50">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-slate-400 mt-1 uppercase tracking-widest">
                              {user.phone ? `Tel: ${user.phone}` : 'Sem telefone'}
                            </p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-5">
                        <div className="space-y-3">
                          <select
                            ...
                            className={`text-[9px] font-black uppercase tracking-wider px-3 py-1.5 rounded-xl border transition-all outline-none ${
                              user.role === 'admin'
                                ...
                                : user.role === 'premium' || user.role === 'elite'
                          ...
                          <div>
                            <p className="mb-2 text-[9px] font-black uppercase tracking-[0.18em] text-slate-400">Pacotes extras</p>
                            <div className="flex flex-wrap gap-2">
                              {ADMIN_EXTRA_PACKAGES.map((pkg) => {
                                const isActive = currentPackages.includes(pkg.id);
                                return (
                                  <button
                                    key={pkg.id}
                                    type="button"
                                    onClick={() => handleToggleUserPackage(user.id, currentPackages, pkg.id)}
                                    className={`rounded-xl px-3 py-1.5 text-[9px] font-black uppercase tracking-[0.18em] transition-all ${
                                      isActive
                      ...
                      <td className="px-6 py-5">
                        <p className="text-xs font-bold text-slate-700">{user.areas?.name || '---'}</p>
                      </td>
                      <td className="px-6 py-5">
                        <p className="text-xs font-bold text-slate-700">{user.phone || 'Sem telefone'}</p>
                      </td>
                      <td className="px-6 py-5">
                        <p className="text-xs font-bold text-slate-700 break-all">{user.email || 'Sem email'}</p>
                      </td>
                      <td className="px-6 py-5">
                        <div className="flex items-center gap-2">
                          <div className={`h-2 w-2 rounded-full ${user.last_active && (Date.now() - new Date(user.last_active).getTime() < 10 * 60 * 1000) ? 'bg-emerald-500 animate-pulse' : 'bg-slate-300'}`} />
                      ...
                      <td className="px-6 py-5 text-right rounded-r-2xl border border-transparent border-r-slate-200 group-hover:border-slate-100">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            onClick={async () => {
                              try {
                                ...
                                const msg = data
                                  ?.map((log: any) => `[${new Date(log.activity_date).toLocaleTimeString()}] ${log.activity_type}`)
                                  .join('\n');
                          ...
                          <select
                            value={user.selected_area_id || ''}
                            onChange={(e) => handleUpdateUserArea(user.id, e.target.value)}
